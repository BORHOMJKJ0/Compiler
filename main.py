...
FROM Customers;
    ...
    # 🔹 Print all tokens
    print("=== TOKENS ===")
    lexer.reset()  # reset before printing
    all_tokens = lexer.getAllTokens()
    for t in all_tokens:
        token_type_name = lexer.symbolicNames[t.type] if t.type >= 0 else "<EOF>"
        print(f"{t.text!r}  ->  {token_type_name} (line={t.line}, col={t.column})")
    lexer.reset() 
